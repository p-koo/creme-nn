...
save_path = '../results/cre_distance_test_enhancers.pickle'
...
for i, row in tqdm(cre_df.iterrows(), total=len(cre_df)):
    ...
    # get coordinates for cre of interest
    cre_tile = [row['tile_start'], row['tile_start']+window]

    # perform TSS-CRE distance dependence Test
    pred_control, pred_mut = creme.distance_test(model, x, tss_tile, cre_tile, other_tiles, num_shuffle, mean=True)
