...
def main():
    ...
    if model_name.lower() == 'enformer':
        ...
        model = borzoi_custom_model.Borzoi('../data/borzoi/*/*', track_index=track_index, aggregate=False)
    ...
    model_results_dir = utils.make_dir(
        f"{utils.make_dir(f'{result_dir}/context_dependence_test_{N_shuffles}')}/{model_name}")  # output of this test
    ...
    if model_name == 'enformer':
        ...
        summary_combined.to_csv(
            f'{csv_dir}/context_dependence_test.csv')  # summary of context effect for every selected gene
